...
function enemy.new(x, y)
    ...
    e.speed, e.speedMod, e.xVel = 100, 1, 100
    e.rageCounter, e.rageTrigger = 0, 3
    e.damage, e.state = 1, "walk"

    e.width  = sprites.enemy.walk[1]:getWidth()
    e.height = sprites.enemy.walk[1]:getHeight()
    e.animation = {
        timer = 0, rate = 0.1,
        run  = { total = 4, current = 1, img = enemy.runAnim },
...
function enemy:incrementRage()
    ...
    if self.rageCounter > self.rageTrigger then
        self.state, self.speedMod, self.rageCounter = "run", 3, 0
    else
        self.state, self.speedMod = "walk", 1
    end
end
...
function enemy:animate(dt)
    ...
    if self.animation.timer > self.animation.rate then
        ...
        local a = self.animation[self.state]
...
function enemy:syncPhysics()
    ...
    self.physics.body:setLinearVelocity(self.xVel * self.speedMod, vy)
end

function enemy:draw()
    local sx = (self.xVel < 0) and -1 or 1
    love.graphics.draw(
        self.animation.draw,
...
function enemy.updateAll(dt)
    for _, e in ipairs(activeEnemies) do e:update(dt) end
end

function enemy.drawAll()
    for _, e in ipairs(activeEnemies) do e:draw() end
end
